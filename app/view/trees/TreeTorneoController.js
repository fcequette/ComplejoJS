...
Ext.define('Torneo.view.trees.TreeTorneoController', {
  ...
  ,onEditClick:function (btn , e){
    if(Ext.cq1('treetorneo').getSelectionModel().selected ){
      // if( !Ext.cq1('treetorneo').getSelectionModel().selected.items[0].data.isFirst) {
         ...
         if(Ext.isDefined(v)){
                ...
                ,defaults:{
                   ...
                   ,listeners:{
                     keypress: function(key,e){
                       if (e.keyCode==13){
                         console.log('holi');
                           Ext.cq1('#saveEditTorneo').fireEvent('click', Ext.cq1('#saveEditTorneo'))
                       }
                     }
                   }
                }
              ...
              ,listeners:{
                afterRender: function (win,e) { //Editar
                  ...
                  Ext.defer(function(){win.down('#winEditDescri').focus(true)},100);
...
,onDeleteClick:function (btn, e) {
  if( !Ext.cq1('treetorneo').getSelectionModel().selected.items[0].data.root){
    ...
    if(Ext.isDefined(ventana)){
        ...
        ,listeners:{
          afterRender: function (win,e){
            ...
            Ext.defer(function(){win.down('#deleteBtn').focus(true)},100);
    ...
    Ext.create('Ext.window.Window', {
      ...
      ,defaults:{
         ...
         ,listeners:{
           keypress: function(key,e){
             if (e.keyCode==13){
                 Ext.cq1('#saveAltaTorneo').fireEvent('click', Ext.cq1('#saveAltaTorneo'))
             }
           }
         }
      }
      ...
      ,listeners:{
        afterRender: function (win,e){
          ...
          Ext.defer(function(){win.down('#winAltaDescri').focus(true)},100);
          ...
          switch (ventana) {
            ...
            break;
            default:
              Ext.ComponentQuery.query('#botonAdd')[0].urlAlta = '/api/torneo';
            break;
          }
        }
      }
...
,onItemClick:function (est, record, item, index, e, eOpts ){
  ...
  if(record.data.leaf) {
     Ext.ComponentQuery.query('#botonDelete')[0].urlDelete = '/api/equipozona/'+record.parentNode.data.zona_id+'-'+record.data.equipo_id;
  ...
  if(record.data.root){
    ...
    Ext.ComponentQuery.query('#botonDelete')[0].record = record;
    Ext.ComponentQuery.query('#botonEdit')[0].urlEdit = '/api/torneo';
    Ext.ComponentQuery.query('#botonAdd')[0].urlAlta = '/api/torneo';
    Ext.ComponentQuery.query('#botonDelete')[0].urlDelete = '/api/torneo';
  ...
  if(record.data.nivel == 1 ){
    ...
    Ext.ComponentQuery.query('#botonEdit')[0].urlEdit = '/api/torneo';
    Ext.ComponentQuery.query('#botonAdd')[0].urlAlta = '/api/categoria';
    Ext.ComponentQuery.query('#botonDelete')[0].urlDelete = '/api/torneo';
  ...
  if(record.data.nivel == 2){
    ...
    Ext.ComponentQuery.query('#botonAdd')[0].record = record;
    Ext.ComponentQuery.query('#botonEdit')[0].urlEdit = '/api/categoria';
    Ext.ComponentQuery.query('#botonAdd')[0].urlAlta = '/api/zona';
    Ext.ComponentQuery.query('#botonDelete')[0].urlDelete = '/api/categoria';
  ...
  if(record.data.nivel == 3){
    ...
    Ext.ComponentQuery.query('#botonEdit')[0].urlEdit = '/api/zona';
     Ext.ComponentQuery.query('#botonDelete')[0].urlDelete = '/api/zona';
